...
class AdminSearchDeliverymanWindow(QWidget, Ui_admin_search_deliveryman):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.sql=Sql()
    ...
    def searchDeliveryman(self):
        ...
        if not workId and not tel:
            # 如果两者都没有输入，则查询所有
            statement = 'SELECT * FROM deliveryman'
        elif workId and not tel:
            # 如果只输入了工号，则按工号查询
            statement = f"SELECT * FROM deliveryman WHERE delivery_id = '{workId}'"
        elif not workId and tel:
            # 如果只输入了电话，则按电话查询
            statement = f"SELECT * FROM deliveryman WHERE delivery_phone = '{tel}'"
        else:
            # 如果两者都输入了，则构建一个合适的查询语句（根据你的需求来定）
            statement = f"SELECT * FROM deliveryman WHERE delivery_id = '{workId}' AND delivery_phone = '{tel}'"
        ...
        if result:
            ...
            for row_num, row_data in enumerate(result):
                ...
                item4 = QTableWidgetItem(str(row_data[7]))
...
if __name__ == "__main__":
    ...
    # 初始化并展示我们的界面组件
    window = AdminSearchDeliverymanWindow()
    # 设置窗口图标
