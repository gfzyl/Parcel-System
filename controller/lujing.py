...
def lujing2(chufa, mudi, chengshi):  # 输入出发与到达市,通过递归  贪心算法  计算出最短路径   只供 lujing()调用
    statement = f"select 省会城市 from adjacency where {chufa}=1"  # 编辑sql命令
    xlcs = sql.execute_query(statement)
    # print(chufa,'-',mudi,"附近的城市",xlcs)
    statement = f"select longitude,latitude from city where city_name ='{mudi}'"
...
def lujing(chufa, mudi):  # 函数功能是输入2省 计算出最优路径
    if chufa == mudi:  # 如果是本省快递,返回0
        ...
        statement = f"select capital_city from province where prv_name='{chufa}'"
        shcs = sql.execute_query(statement) # shcs 省会城市

        statement = f"select capital_city from province where prv_name='{mudi}'"
